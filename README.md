...
2. Na pasta src/main/resources/ do projeto, existe um arquivo `queue-config.yaml` onde podem ser configuradas todas as propriedades do simulador. As propriedades deste arquivo que são necessárias para a execução do trabalho estão descritas abaixo:
    - **queues:** enumera as filas utilizadas pelo simulador e suas características principais (capacidade, número de servidores, e intervalos de chegada e de atendimento de clientes). O nome de cada uma destas filas deve iniciar com Q, seguido por uma numeração (por exemplo: Q2)
